...
const AddKursi = (myProp: props) =>{
    ...
    return(
        <div>
            <ToastContainer
            containerId={`toastAddKursi`}/>
            <button type="button" onClick={()=>openModal()} className="px-9 py-6 bg-green-700 hover:bg-green-600 rounded-md text-white text-lg font-bold my-2">+</button>
            <Modal isShow={show}>
            <form onSubmit={handleSubmit}>
                    <div className="w-full p-3 rounded-t-lg">
                        <h1 className="font-semibold text-lg">Tambah Kursi</h1>
                        <span className="text-sm text-slate-500">Pastikan data yang diisi sudah benar</span>
                    </div>
                    <div className="w-full p-3">
                        <div className="my-2 border rounded-md p-3">
                            <small className="text-sm font-semibold text-sky-600">Nomer Kursi</small>
