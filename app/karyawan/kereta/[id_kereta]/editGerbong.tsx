            ...
            <Modal isShow={show}>
                <form onSubmit={e=>handleSubmit(e)}>
                    ...
                    <div className="w-full p-3 rounded-t-lg">
                        <h1 className="font-semibold text-lg">Edit Gerbong Kereta</h1>
                        <span className="text-sm text-slate-500">Pastikan data yang diisi sudah benar</span>
                    </div>
                    {/* modal body */}
                    <div className="w-full p-3">
                        <div className="my-2 border rounded-md p-3">
                            <small className="text-sm fonst-semibold text-sky-600">Nama Gerbong</small>
                            <input type="text" id={`name-${myProp.item.id}`} value={name} onChange={e=>setName(e.target.value)} required={true} className="w-full p-1 outline-none focus:border-b-sky-600 focus:border-b"/>
                        </div>
                        <div className="my-2 border rounded-md p-3">
                            <small className="text-sm fonst-semibold text-sky-600">Jumlah kursi</small>
                            <input type="number" id={`seat_count-${myProp.item.id}`} value={seat_count.toString()} onChange={e=>setSeatCount(Number(e.target.value))} required={true} className="w-full p-1 outline-none focus:border-b-sky-600 focus:border-b"/>
                        </div>
                    </div>
                    {/* Modal footer */}
