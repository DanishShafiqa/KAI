...
const AddSchedule = (myProp:Props) => {
    ...
    return(
        <div>
            ...
            <button className="px-4 rounded-md text-white bg-green-600 hover:bg-green-500 py-2" type="button" onClick={()=>openModal()}>Tambah Jadwal Kereta</button>
            <Modal isShow={show}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="w-full p-3">
                        ...
                        <div className="p-2 my-2 border rounded-md">
                            <small className="text-xs font-semibold text-sky-500">Jenis Kereta</small>
                            <select name="" id={`train_id`} value={train_id.toString()} onChange={e=>setTrainId(Number(e.target.value))} className="w-full outline-none p-1 border hover:bordert-sky-500" required={true}>
                                <option value="">Pilih Jenis Kereta</option>
                                {myProp.trains.map((kereta, index)=>(<option value={kereta.id} key={`optionKereta-${index}`}>{kereta.name}</option>))}
                            </select>
                        </div>
                    </div>
                    {/* Modal footer */}
