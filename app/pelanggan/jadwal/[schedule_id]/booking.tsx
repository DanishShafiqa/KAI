...
const Booking =(myProp: Props)=>{
    ...
    useEffect(()=>
        ...
        {setWagons([...myProp.wagons])},[myProp])
    ...
    const handleAddSeat=(seatBook: SeatBook)=>{
        ...
        const findWagonIndex = tempWagon.findIndex(item => item.seats.map(it=> it.seat_number).includes(seatBook.seat_number))
        // mencari posisi index dari kursi yang dipilih
        const findSeatIndex = tempWagon[findWagonIndex].seats.findIndex(item=>item.seat_number=== seatBook.seat_number)
        // ubah status "used" mmenjadi true
    ...
    const handleRemoveSeat=(index: number, seatBook: SeatBook)=>{
        const temp = [...details]
        temp.splice(index, 1)
        //splice= menghapus data pada array
        setDetails(temp)
        ...
        // mencari posisi index dari kursi yang dipilih
        const findSeatIndex = tempWagon[findWagonIndex].seats.findIndex(item=>item.seat_number=== seatBook.seat_number)
        // ubah status "used" mmenjadi false
        ...
        <div>
            ...
            {myProp.wagons.map((item, index)=>(
                <div key={`keyWagon-${index}`} className="w-full my-2 p-3 rounded-md shadow-md border">
                    ...
                    <div className="flex flex-wrap gap-3">{item.seats.map((seat, indexSeat)=>(
                        <Seat key={`keySeat-${index}-${indexSeat}`} item={seat} onSave={seatBook=> handleAddSeat(seatBook)}/>
                    ))}</div>
                </div>
            ))}
            <button type="button" onClick={()=> handleSave()} className="w-full p-3 rounded-md my-2 bg-green-600 hover:bg-green-500 text-white">Pesan Sekarang</button>
        </div>
    )
}
