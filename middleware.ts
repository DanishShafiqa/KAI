...
import { verifyKaryawan, verifyPelanggaan } from "./helper/authorization";
...
export const middleware = async(request: NextRequest)=>{
    ...
    if(request.nextUrl.pathname.startsWith(`/pelanggan`)){
        ...
        const isVerifiedToken = await verifyPelanggaan(token ?? "")
        if(!isVerifiedToken) return NextResponse.redirect(redirectLogin)
        return NextResponse.next()
    }
    return NextResponse
...
// menentukan role mana saja yang akan memberlakukan proses middleware
